...
// IMPORTAR TUS IMÁGENES - reemplaza estas rutas con las tuyas
...
export default function Inclusion() {
  ...
  // Duplicamos los logos suficientes veces para evitar espacios vacíos
  const duplicatedLogos = [...logos, ...logos, ...logos, ...logos];

  return (
    <section className="w-full bg-[#edf9e5] py-20 px-6 font-causten">
      ...
      <div className="relative overflow-hidden w-full py-6">
        ...
        {/* CARRUSEL CON LOOP CONTINUO - MÁS RÁPIDO */}
            ...
            >
              <img
                ...
                className={`h-14 w-auto transition-all duration-300 ${
                  hoveredIndex === idx 
                    ? 'scale-110 brightness-110 drop-shadow-lg' // Resaltar al hacer hover
                    : 'opacity-90 hover:opacity-100' // Colores normales con opacidad suave
                }`}
                alt={`Logo ${idx}`}
              />
            </div>
        ...
        <div className="flex gap-6 items-start">
          <div className="flex-shrink-0">
            <img 
              src={iconoInnovacion} 
              alt="Innovación" 
              className="w-25 h-25"  /* Ajusta el tamaño según necesites */
            />
          </div>
          <div>
